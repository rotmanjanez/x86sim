...
const char* fu_names[FU_COUNT] = {
    "alu1", "aluc", "alu2", "lsu1", "alu3", "lsu2", "fadd", "fmul", "fcvt",
};
...
#define foreach_list_mutable_linktype(L, obj, entry, nextentry, linktype)                                              \
  linktype* entry;                                                                                                     \
  linktype* nextentry;                                                                                                 \
  for (entry = (L).next, nextentry = entry->next, __builtin_prefetch(entry->next), obj = (typeof(obj))entry; entry != &(L);      \
       entry = nextentry, nextentry = entry->next, __builtin_prefetch(nextentry), obj = (typeof(obj))entry)
...
#define foreach_issueq(expr)                                                                                           \
  {                                                                                                                    \
    ...
    core.issueq_int2.expr;                                                                                             \
    core.issueq_int3.expr;                                                                                             \
    core.issueq_fp.expr;                                                                                               \
  }
...
#define issueq_operation_on_cluster_with_result(core, cluster, rc, expr)                                               \
  ...
  case 0:                                                                                                              \
    ...
    break;                                                                                                             \
  case 1:                                                                                                              \
    rc = core.issueq_int2.expr;                                                                                        \
    break;                                                                                                             \
  case 2:                                                                                                              \
    rc = core.issueq_int3.expr;                                                                                        \
    break;                                                                                                             \
  case 3:                                                                                                              \
...
struct OutOfOrderCoreStats {
  ...
  struct issue {
    ...
    struct width {
      ...
      W64 int2[OutOfOrderModel::MAX_ISSUE_WIDTH + 1];
      W64 int3[OutOfOrderModel::MAX_ISSUE_WIDTH + 1];

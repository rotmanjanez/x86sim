// Host-POSIX half of the Linux syscall support library.
//
// This translation unit implements the syscalls that need the host operating
// system: file, directory, socket, and host-time I/O, plus the per-process host
// fd-table machinery. It is compiled into the separate x86sim::posix library and
// links the portable x86sim::defaults library for the shared decode helpers.
...
// --- static-exec loader (host file I/O; reserved for Phase 2 execve) -------
...
// File-backed mmap copies bytes from a host fd into the guest mapping. The
// portable SysMmap calls this through the loader hook it exposes; we register
// the implementation at startup so x86sim::posix-linked tools keep file-backed
// mmap support. Returns std::nullopt on success or a Linux errno on failure; the
// caller (SysMmap) owns unmapping on failure.
std::optional<int> load_file_backed_mapping(ProcessId pid, AddressSpace& space, address_t dest, word_t length,
                                            word_t fd_arg, word_t offset) noexcept {
  const auto fd = host_fd_for(pid, fd_arg);
  if (!fd)
  ...
  try {
    ...
    while (total < length) {
      ...
      if (bytes_read < 0)
        return detail::host_errno_to_linux(errno);
      if (bytes_read == 0)
        break;

      auto written =
          space.write(dest + total, std::span<const std::byte>(buffer.data(), static_cast<std::size_t>(bytes_read)));
      if (!written)
        return detail::memory_error_to_linux(written.error());
      total += static_cast<word_t>(bytes_read);
    }
  } catch (...) {
    return detail::linux_eio;
  }
  return std::nullopt;
}

const bool mmap_loader_registered = [] {
  detail::set_mmap_file_loader(&load_file_backed_mapping);
  return true;
}();

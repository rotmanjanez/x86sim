...
#define foreach_list_mutable_linktype(L, obj, entry, nextentry, linktype)                                              \
  linktype* entry;                                                                                                     \
  linktype* nextentry;                                                                                                 \
  for (entry = (L).next, nextentry = entry->next, prefetch(entry->next), obj = (typeof(obj))entry; entry != &(L);      \
       entry = nextentry, nextentry = entry->next, prefetch(nextentry), obj = (typeof(obj))entry)

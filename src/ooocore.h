...
#define per_context_ooocore_stats_update(vcpuid, expr)                                                                 \
...
static const char* physreg_state_names[MAX_PHYSREG_STATE] = {"none",    "free", "waiting",    "bypass",
...
struct IssueQueue {
  ...
  bool shared_empty() { return (shared_entries == 0); }

  bool remaining() const { return (size - count); }
  bool empty() const { return (!count); }
  bool full() const { return (!remaining()); }

  int uopof(int slot) const { return uopids[slot]; }
...
#define foreach_list_mutable_linktype(L, obj, entry, nextentry, linktype)                                              \
  linktype* entry;                                                                                                     \
  linktype* nextentry;                                                                                                 \
  for (entry = (L).next, nextentry = entry->next, prefetch(entry->next), obj = (decltype(obj))entry; entry != &(L);    \
       entry = nextentry, nextentry = entry->next, prefetch(nextentry), obj = (decltype(obj))entry)

#define foreach_list_mutable(L, obj, entry, nextentry)                                                                 \
  foreach_list_mutable_linktype(L, obj, entry, nextentry, selfqueuelink)
...
struct StateList : public selfqueuelink {
  ...
  StateList(const char* name, ListOfStateLists& lol, W32 flags = 0) { init(name, lol, flags); }
  ...
  selfqueuelink* peek() { return (empty()) ? null : head(); }
...
struct FetchBufferEntry : public TransOp {
  ...
  FetchBufferEntry(const TransOp& transop) { *((TransOp*)this) = transop; }
};
...
struct ReorderBufferEntry : public selfqueuelink {
  ...
  byte entry_valid : 1, load_store_second_phase : 1, all_consumers_off_bypass : 1, dest_renamed_before_writeback : 1,
      no_branches_between_renamings : 1, transient : 1, lock_acquired : 1, issued : 1;
  byte tlb_walk_level;
  ...
  void changestate(StateList& newqueue, bool place_at_head = false, ReorderBufferEntry* prevrob = null) {
    ...
    if (place_at_head)
      newqueue.enqueue_after(this, prevrob);
    else
  ...
  int select_cluster();
  int issue();
  Waddr addrgen(LoadStoreQueueEntry& state, Waddr& origaddr, Waddr& virtpage, W64 ra, W64 rb, W64 rc,
                PTEUpdate& pteupdate, Waddr& addr, int& exception, PageFaultErrorCode& pfec, bool& annul);
  bool handle_common_load_store_exceptions(LoadStoreQueueEntry& state, Waddr& origaddr, Waddr& addr, int& exception,
                                           PageFaultErrorCode& pfec);
  int issuestore(LoadStoreQueueEntry& state, Waddr& origvirt, W64 ra, W64 rb, W64 rc, bool rcready,
                 PTEUpdate& pteupdate);
...
struct PhysicalRegister : public selfqueuelink {
  ...
  void changestate(int newstate) {
    if likely (state != PHYSREG_NONE)
      get_state_list(state).remove(this);
    state = newstate;
...
public:
  void reset(W8 threadid, bool check_id = true) {
    if (check_id && this->threadid != threadid)
      return;
...
struct OutOfOrderCoreEvent {
  ...
  union {
    ...
    struct {
      ...
      W16 flags;
      W64 data;
      byte transient : 1, all_consumers_sourced_from_bypass : 1, no_branches_between_renamings : 1,
          dest_renamed_before_writeback : 1;
    } writeback;
    struct {
      IssueState state;
...
struct EventLog {
  ...
  OutOfOrderCoreEvent* add(int type, const RIPVirtPhys& rvp) { return add()->fill(type, rvp); }

  OutOfOrderCoreEvent* add(int type, const FetchBufferEntry& uop) { return add()->fill(type, uop); }

  OutOfOrderCoreEvent* add(int type, const ReorderBufferEntry* rob) { return add()->fill(type, rob); }

  OutOfOrderCoreEvent* add_commit(int type, const ReorderBufferEntry* rob) { return add()->fill_commit(type, rob); }

  OutOfOrderCoreEvent* add_load_store(int type, const ReorderBufferEntry* rob, LoadStoreQueueEntry* inherit_sfr = null,
                                      Waddr addr = 0) {
...
enum { ROB_STATE_READY = (1 << 0), ROB_STATE_IN_ISSUE_QUEUE = (1 << 1), ROB_STATE_PRE_READY_TO_DISPATCH = (1 << 2) };
...
#else
#define InitClusteredROBList(name, description, flags) name[0](description "-all", rob_states, flags);
#endif
...
struct ThreadContext {
  ...
  StateList
      rob_issued_list[MAX_CLUSTERS]; // Issued and in progress (or for loads, returned here after address is generated)
  StateList rob_completed_list[MAX_CLUSTERS]; // Completed and result in transit for local and global forwarding
  StateList rob_ready_to_writeback_list
      [MAX_CLUSTERS];              // Completed; result ready to writeback in parallel across all cluster register files
...
#define foreach_issueq(expr)                                                                                           \
  {                                                                                                                    \
    OutOfOrderCore& core = getcore();                                                                                  \
    core.issueq_int0.expr;                                                                                             \
    core.issueq_int1.expr;                                                                                             \
    core.issueq_ld.expr;                                                                                               \
    core.issueq_fp.expr;                                                                                               \
  }
...
#define issueq_operation_on_cluster_with_result(core, cluster, rc, expr)                                               \
  switch (cluster) {                                                                                                   \
  case 0:                                                                                                              \
    rc = core.issueq_int0.expr;                                                                                        \
    break;                                                                                                             \
  case 1:                                                                                                              \
    rc = core.issueq_int1.expr;                                                                                        \
    break;                                                                                                             \
  case 2:                                                                                                              \
    rc = core.issueq_ld.expr;                                                                                          \
    break;                                                                                                             \
  case 3:                                                                                                              \
    rc = core.issueq_fp.expr;                                                                                          \
    break;                                                                                                             \
  }
...
#define foreach_issueq(expr)                                                                                           \
  { getcore().issueq_all.expr; }
  void sched_get_all_issueq_free_slots(int* a) { a[0] = issueq_all.remaining(); }
...
#define issueq_operation_on_cluster(core, cluster, expr)                                                               \
  {                                                                                                                    \
    int dummyrc;                                                                                                       \
    issueq_operation_on_cluster_with_result(core, cluster, dummyrc, expr);                                             \
  }
...
struct formatter<OutOfOrderModel::MemoryInterlockEntry> {
  ...
  auto format(const OutOfOrderModel::MemoryInterlockEntry& entry, format_context& ctx) const {
    return format_to(ctx.out(), "uuid={} rob={} vcpu={} thread={}", entry.uuid, entry.rob, entry.vcpuid,
                     entry.threadid);
  }
};
...
struct formatter<OutOfOrderModel::MemoryInterlockBuffer> {
  constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }
  auto format(const OutOfOrderModel::MemoryInterlockBuffer& buf, format_context& ctx) const {
    auto out = ctx.out();
    for (int set = 0; set < 16; set++) {
      const auto& s = buf.sets[set];
      for (int way = 0; way < 4; way++) {
        if (s.tags.tags[way] == s.tags.INVALID)
          continue;
        out = format_to(out, "  set {} way {} addr {} {}\n", set, way, (void*)(Waddr)s.tags.tags[way], s.data[way]);
      }
    }
    return out;
  }

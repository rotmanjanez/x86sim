...
void odstream::set_ringbuf_mode(bool new_ringbuf_mode) {
  if (fd < 0)
    return;
  if (ringbuf_mode == new_ringbuf_mode)
    return;
...
int odstream::write(const void* data, int count) {
  ...
  if unlikely (force_synchronous_streams)
    flush();
...
stringbuf& operator<<(stringbuf& os, const bitmaskstring& bs) {
  if (bs.reverse) {
    // LSB first:
    for (int i = 0; i < bs.n; i++)
      os << (char)(bit(bs.mask, i) ? (bit(bs.bits, i) + '0') : 'x');
  } else {
    // MSB first (default):
    for (int i = bs.n - 1; i >= 0; i--)
      os << (char)(bit(bs.mask, i) ? (bit(bs.bits, i) + '0') : 'x');

...
struct range {
  ...
  ostream& print(ostream& os) const { return os << '[', lo, ' ', hi, ']'; }
};
...
struct FixedIntegerQueueSet {
  ...
  bool isnull(T tag) const { return (tag == nulltag); }
  ...
  void add(int qid, T tag) {
    ...
    if likely (!isnull(tail))
      next[tail] = tag;
    tail = tag;
    if unlikely (isnull(head))
      head = tag;
  }
  ...
  T dequeue(int qid) {
    ...
    head = next[tag];
    next[tag] = nulltag;
    if unlikely (tail == tag)
      tail = nulltag;
    return tag;
  }

  void splice_into_head(int destqid, int srcqid) {
    if unlikely (empty(srcqid))
      return;
  ...
  bool empty(int qid) const { return (isnull(heads[qid])); }
...
struct indexrefnull {
  ...
  T* operator->() const { return (index >= 0) ? &(get(index)) : null; }

  T& operator*() const { return (index >= 0) ? get(index) : *(T*)null; }
...
struct bitvecbase {
  ...
  bitvecbase(const bitvecbase<N>& vec) {
    foreach (i, N)
      w[i] = vec.w[i];
  }
  ...
  void shiftleftop(size_t shift) {
    if likely (shift) {
      ...
      if unlikely (offset == 0) {
        ...
        for (size_t i = N - 1; i > wshift; --i) {
          w[i] = (w[i - wshift] << offset) | (w[i - wshift - 1] >> suboffset);
        }
        w[wshift] = w[0] << offset;
      }
  ...
  void maskop(size_t count) {
    T m = (!count) ? 0 : (count % BITS_PER_WORD) ? ((T(1) << bitof(count)) - T(1)) : T(-1);
  ...
  size_t msbop(size_t notfound) const {
    for (int i = N - 1; i >= 0; i--) {
      T t = w[i];
      if likely (t)
        return (i * BITS_PER_WORD) + __builtin_clzl(t);
    }
    return notfound;
  }

  // assume value is nonzero
  size_t lsbop() const { return lsbop(0); }

  // assume value is nonzero
  size_t msbop() const { return msbop(0); }
...
struct bitvecbase<1> {
  ...
  void accumop(size_t i, size_t n, T v) { w |= (v << i); }
...
struct bitvec_sanitizer {
  static void sanitize(unsigned long& val) { val &= ~((~static_cast<unsigned long>(0)) << extrabits); }
};
...
public:
  ...
  public:
    ...
    // For x = b[i];
    inline operator bool() const { return x86_bt(*wp, bpos); }
  ...
  bitvec<N> rotright(int index) const { return ((*this) >> index) | ((*this) << (N - index)); }

  bitvec<N> rotleft(int index) const { return ((*this) << index) | ((*this) >> (N - index)); }
  ...
  bool test(size_t index) const { return (this->getword(index) & base_t::maskof(index)) != static_cast<T>(0); }
  ...
  bitvec<N> operator%(size_t b) const { return mask(b); }

  bitvec<N> extract(size_t index, size_t count) const { return (bitvec<N>(*this) >> index) % count; }

  bitvec<N> operator()(size_t index, size_t count) const { return extract(index, count); }

  bitvec<N> operator&(const bitvec<N>& y) const { return bitvec<N>(*this) &= y; }
...
struct BitmapAllocator3Level {
  ...
  void reset(size_t sizelimit = N) {
    ...
    foreach (i, lengthof(level2)) {
      level2[i]++;
    }
    foreach (i, lengthof(level1)) {
      level1[i]++;
    }
    highest_count = 0;
  }
  ...
  W64s alloc(W64 index) {
    ...
    if unlikely (!level1[slot1])
      level2[slot2][offset2] = 0;
  ...
  void free(W64 index) {
    ...
    W64 offset2 = bits(index, 6, 6);
    if unlikely (*level1[slot1])
      level2[slot2][offset2] = 1;

    W64 slot3 = 0; // a.k.a. bits(index, 18, 6);
    W64 offset3 = bits(index, 12, 6);
    if unlikely (*level2[slot2])
      level3[slot3][offset3] = 1;
  }
  ...
  bool isused(W64 index) const { return (!isfree(index)); }

  ostream& print(ostream& os) const {
    os << "FreeBitmap3Level:", endl;
    os << "  L3:";
    foreach (i, lengthof(level3))
      os << ' ', level3[i], ((i % 4 == 3) ? '\n' : ' ');
    os << endl;
    os << "  L2:";
    foreach (i, lengthof(level2))
...
struct Hashtable
    : public SelfHashtable<K, ObjectHashtableEntry<K, T, KM>, setcount, ObjectHashtableLinkManager<K, T, KM>> {
  typedef ObjectHashtableEntry<K, T, KM> entry_t;
  ...
  struct Iterator : public base_t::Iterator {
    ...
    Iterator(Hashtable<K, T, setcount, KM>* ht) { reset(ht); }

    Iterator(Hashtable<K, T, setcount, KM>& ht) { reset(ht); }

    void reset(Hashtable<K, T, setcount, KM>* ht) { base_t::Iterator::reset(ht); }

    void reset(Hashtable<K, T, setcount, KM>& ht) { base_t::Iterator::reset(&ht); }

    KeyValuePair<K, T>* next() { return base_t::Iterator::next(); }
  };
...
struct FixedValueHashtable {
  ...
  static int setof(T value) { return foldbits<log2(setcount)>(value); }

  int lookup(T value) const {
    int set = setof(value);
    ptr_t slot = sets[set];
    while (slot >= 0) {
      if unlikely (data[slot] == value)
        return slot;
      slot = next[slot];
    }
    return -1;
  ...
  bool contains(T value) const { return (lookup(value) >= 0); }
  ...
  ostream& print(ostream& os) const {
    os << "FixedValueHashtable<", sizeof(T), "-byte data, ", N, " slots, ", setcount, " sets> containing ", count,
        " entries:", endl;
    foreach (i, count) {
      T v = data[i];
...
struct FixedKeyValueHashtable {
  ...
  static int setof(Tkey key) { return foldbits<log2(setcount)>(key); }

  int lookup(Tkey key) const {
    int set = setof(key);
    ptr_t slot = sets[set];
    while (slot >= 0) {
      if unlikely (keys[slot] == key)
        return slot;
      slot = next[slot];
    }
    return -1;
  ...
  ostream& print(ostream& os) const {
    os << "FixedKeyValueHashtable<", sizeof(Tkey), "-byte key, ", sizeof(Tdata), "-byte data, ", N, " slots, ",
        setcount, " sets> containing ", count, " entries:", endl;
    foreach (i, count) {
      Tkey k = keys[i];
      Tdata v = data[i];
...
struct GenericChunkList {
  ...
  bool empty() const { return bytes_are_all_zero(&list, sizeof(list)); }
...
struct SearchableChunkList16Entry {
  ...
  SearchableChunkList16Entry() { reset(); }
...
bool detect_repeated_pattern(const T* list, int n, int& pattern_length, int& repeat_count, int& remaining_count) {
  ...
  if likely ((repeat_count == 1) && (pattern_length == 1))
    return false;

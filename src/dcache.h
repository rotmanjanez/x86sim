...
template<int tlbid, int size>
struct formatter<CacheSubsystem::TranslationLookasideBuffer<tlbid, size>> {
  constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }
  auto format(const CacheSubsystem::TranslationLookasideBuffer<tlbid, size>& tlb, format_context& ctx) const {
    auto out = ctx.out();
    out = std::format_to(out, "TLB<{}>: {} of {} entries valid\n", tlbid, tlb.valid.count(), size);
    out = std::format_to(out, "  Valid:  {}\n", tlb.valid.to_string());
    foreach (i, size) {
      if (!tlb.valid[i])
        continue;
      W64 tag = tlb.tagsmirror[i];
      W64 virtpage = (tag & ((1ULL << 36) - 1)) << 12;
      W64 threadid = tag >> 36;
      out = std::format_to(out, "  slot {:>3}: virt page {} thread {}\n", i, (void*)(Waddr)virtpage, threadid);
    }
    return out;
  }
};

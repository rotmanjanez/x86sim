...
set dir [pwd]
if {[info exists env(SQLITE_TESTDIR)]} {
  set testdir [file normalize $env(SQLITE_TESTDIR)]
} else {
...
proc find_interpreter {} {
  ...
  if {$rc} {
    if { [string match -nocase testfixture* $interpreter]==0
      ...
      set status [catch {
        set cmd [trd_early_x86sim_command [trd_get_bin_name testfixture]]
        exec {*}$cmd [info script] {*}$::argv >@ stdout
      } msg]
      exit $status
    }
  }
...
Usage: 
  ...
  where SWITCHES are:
    ...
    --x86sim PATH            Run testfixture through this x86sim-linux binary
    --x86sim-arg ARG         Extra argument passed to x86sim-linux; may repeat
    --x86sim-core CORE       Core passed to x86sim-linux, e.g. seq or ooo
...
set TRG(x86sim) [expr {[info exists env(X86SIM)] ? $env(X86SIM) : ""}]
set TRG(x86sim_core) [expr {[info exists env(X86SIM_CORE)] ? $env(X86SIM_CORE) : "seq"}]
set TRG(x86sim_args) [expr {[info exists env(X86SIM_ARGS)] ? $env(X86SIM_ARGS) : ""}]

proc trd_shell_quote {arg} {
  if {[regexp {^[A-Za-z0-9_@%+=:,./-]+$} $arg]} {
    return $arg
  }
  return "'[string map {' '\\''} $arg]'"
}

proc trd_x86sim_wrap_testfixture {testfixture} {
  global TRG
  if {$TRG(x86sim)==""} {
    return $testfixture
  }

  set cmd [list $TRG(x86sim) "--core=$TRG(x86sim_core)"]
  foreach arg $TRG(x86sim_args) {
    lappend cmd $arg
  }
  lappend cmd $testfixture

  set quoted [list]
  foreach arg $cmd {
    lappend quoted [trd_shell_quote $arg]
  }
  return [join $quoted " "]
}
...
for {set ii 0} {$ii < [llength $argv]} {incr ii} {
  ...
  if {[string range $a 0 0]=="-"} {
    ...
    } elseif {$a=="--x86sim-core"} {
      incr ii
      set TRG(x86sim_core) [lindex $argv $ii]
    } elseif {[string match "--x86sim-core=*" $a]} {
      set TRG(x86sim_core) [string range $a 14 end]
    } elseif {$a=="--x86sim-arg"} {
      incr ii
      lappend TRG(x86sim_args) [lindex $argv $ii]
    } elseif {[string match "--x86sim-arg=*" $a]} {
      lappend TRG(x86sim_args) [string range $a 13 end]
    } elseif {$a=="--x86sim"} {
      incr ii
      set TRG(x86sim) [file normalize [lindex $argv $ii]]
    } elseif {[string match "--x86sim=*" $a]} {
      set TRG(x86sim) [file normalize [string range $a 9 end]]
...
proc add_tcl_jobs {build config patternlist {shelldepid ""}} {
  ...
  if {[lindex $build 2]=="Valgrind"} {
    ...
    set testfixture [trd_x86sim_wrap_testfixture $testfixture]
  }
...
proc add_zipvfs_jobs {} {
  ...
  foreach s [zipvfs_testrunner_files] {
    set cmd "[trd_x86sim_wrap_testfixture [file join [lindex $bld 1] $TRG(testfixture)]] $s"
    add_job                                  \
        -displaytype tcl                     \
